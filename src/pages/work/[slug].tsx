...
const WorkPage: NextPageWithLayout<WorkPageProps> = (props) => {
  ...
  return (
    <>
      ...
      <CardList slug={slug}>
        {props.cards.map(
          ({ id, slider, slug, project_date, title, description, specs }) => (
            <Card key={id} className={styles.card}>
              ...
              <CardBody>
                ...
                <ul className={styles.specs}>
                  {!!specs &&
                    specs.map(({ spec, id }) => <li key={id}>{spec}</li>)}
                </ul>
              </CardBody>
              <CardFooter>
                <CustomLink
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  const [introList, projects] = await Promise.all([
    getIntro({ area: "work" }),
    getProjects(),
  ]);
  ...
  const [codeProjects, designProjects, illustrationProjects, otherProjects] =
    await Promise.all([
...
// Helper
const filterProjectsByType = async (
